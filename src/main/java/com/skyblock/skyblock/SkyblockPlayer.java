...
public class SkyblockPlayer {
    ...
    public static void registerPlayer(UUID uuid, PlayerJoinEvent event, Consumer<SkyblockPlayer> after) {
        ...
        if (player.getProfile() == null) {
            ...
            player.setProfile(randomFruitProfile);
            player.saveToDisk();  // Save the updated profile to disk
        }
    ...
    private SkyblockPlayer(UUID uuid) {
        ...
        this.profile = FruitProfileGenerator.generateRandomFruitName();
        ...
        this.bossBar = null;
        this.pet = null;
        this.previousArmorContents = null;
    ...
    public void tick() {
        ...
        new BukkitRunnable(){
            ...
            public void run() {
                ...
                GodspunkyPlayer data = GodspunkyPlayer.getUser(getBukkitPlayer());
                try {
                    if (data != null) {
                        PlayerRank rank = data.rank;
                        getBukkitPlayer().setDisplayName(ChatColor.translateAlternateColorCodes('&', rank.getPrefix()) +" "+ getBukkitPlayer().getName());
                        getBukkitPlayer().setPlayerListName(ChatColor.translateAlternateColorCodes('&', rank.getPrefix()) +" "+ getBukkitPlayer().getName());
                    }
                }catch (Exception ex){
                    ex.printStackTrace();
                }
                ...
                IChatBaseComponent footer = new ChatComponentText(
                        "\n" + ChatColor.GREEN + "" + ChatColor.BOLD + "Active Effects\n" + "" +
                                ...
                                ChatColor.RESET + " " + ChatColor.GREEN + "Ranks, Boosters, & MORE!");
       ...
       new BukkitRunnable(){
           ...
           public void run() {
               if (!hand.equals(bukkitPlayer.getItemInHand())) {
                   ...
                   Bukkit.getPluginManager().callEvent(new SkyblockPlayerItemHeldChangeEvent(SkyblockPlayer.getPlayer(bukkitPlayer), hand, itemStack));

                   hand = itemStack;
               }
               if (getQuestLine() != null) {
                   Objective objective = getQuestLine().getObjective(SkyblockPlayer.getPlayer(bukkitPlayer));

                   if (objective != null) {
                       bossBar.setMessage(ChatColor.WHITE + "Objective: " + ChatColor.YELLOW + objective.getDisplay() + " " + objective.getSuffix(SkyblockPlayer.getPlayer(bukkitPlayer)));
                       bossBar.update();
                   } else {
                       bossBar.reset();
                   }
        ...
        if (bukkitPlayer.getLocation().getY() <= -11) {
            ...
            if (isOnIsland()) {
                bukkitPlayer.performCommand("is");
            }
        }
        ...
        //modifyPlayerStats(bukkitPlayer); // todo : enable this method when it fully tested
        tick++;
    }
    public void statsTick(){
        ItemStack itemInHand = bukkitPlayer.getItemInHand();
        if (itemInHand != previousItemInHand) {
            updateStats(previousItemInHand, itemInHand);
            previousItemInHand = itemInHand;
        }

        ItemStack[] armorContents = bukkitPlayer.getInventory().getArmorContents();
        if (!Arrays.equals(previousArmorContents, armorContents)) {
            for (int i = 0; i < armorContents.length; i++) {
                ItemStack previousArmor = previousArmorContents != null && i < previousArmorContents.length
                        ? previousArmorContents[i]
                        : null;
                ItemStack currentArmor = armorContents[i];
                updateStats(previousArmor, currentArmor);
            }
            previousArmorContents = armorContents;
        }
    }

    public void modifyPlayerStats(Player bukkitPlayer) {
        CraftPlayer craftPlayer = (CraftPlayer) bukkitPlayer;
        EntityPlayer entityPlayer = craftPlayer.getHandle();

        double walkSpeed = Math.min((float) (getStat(SkyblockStat.SPEED) / 500.0), 1.0f);
        double maxHealth = Math.round(Math.min(40.0, 20.0 + ((getStat(SkyblockStat.MAX_HEALTH) - 100.0) / 25.0)));
        double health = Math.min(Math.max(1, bukkitPlayer.getMaxHealth() * ((double) getStat(SkyblockStat.HEALTH) / (double) getStat(SkyblockStat.MAX_HEALTH))), bukkitPlayer.getMaxHealth());

        entityPlayer.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED).setValue(walkSpeed);
        entityPlayer.getAttributeInstance(GenericAttributes.maxHealth).setValue(maxHealth);
        entityPlayer.setHealth((float) health);
    }
    ...
    public void updateStats(ItemStack oldItem, ItemStack newItem) {
        if (Util.isNotSkyblockItem(newItem)) {
            newItem = Util.getEmptyItemBase();
        }
        if (Util.isNotSkyblockItem(oldItem)) {
            oldItem = Util.getEmptyItemBase();
        }
    ...
    public boolean isOnIsland() {
        return bukkitPlayer.getWorld().getName().startsWith(SkyblockIsland.ISLAND_PREFIX);
    }
    ...
    private void initConfig() {
        ...
        if (!configFile.exists()) {
            try {
                ...
                config.set("bank.personal.cooldown", -1);
    ...
    public String getCurrentLocationName() {
        ...
        if (bukkitPlayer.getWorld().getName().startsWith(SkyblockIsland.ISLAND_PREFIX)) loc = "Private Island";

        return loc;
    }

    public boolean isNotOnPrivateIsland() {
        if (Bukkit.getWorld("island-" + bukkitPlayer.getUniqueId().toString()) == null) {
            return true;
        }
        return !bukkitPlayer.getWorld().getName().equals(SkyblockIsland.getWorld(bukkitPlayer).getName());
    }
    ...
    public void onQuit() {
        ...
        SkyblockIsland.getIsland(bukkitPlayer.getUniqueId()).onQuit();
