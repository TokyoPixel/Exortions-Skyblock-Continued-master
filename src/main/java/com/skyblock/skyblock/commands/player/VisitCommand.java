...
public class VisitCommand implements Command, TrueAlias<VisitCommand> {
    ...
    public void execute(Player player, String[] args, Skyblock plugin) {
        ...
        SkyblockPlayer p = SkyblockPlayer.getPlayer(target.getUniqueId());
       ...
       // meta.setLore(Arrays.asList(Util.buildLore(
          //      "&7Players:\n&8 - &7" + target.getName() + "\n\n&7Profile: "+ p.getActiveProfile(target.getUniqueId()) +  "\n\n&7Players: &a" + SkyblockIsland.getWorld(target.getPlayer()).getPlayers().size() + "/5\n&7Server: &8island-" + target.getName().toLowerCase() + "\n\n&eClick to visit!"
    //    )));
        // todo fix it!
