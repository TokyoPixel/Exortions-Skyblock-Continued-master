...
public class LeapingSword extends ListeningItem {
    public LeapingSword() {
        super(plugin.getItemHandler().getItem("LEAPING_SWORD.json"), "leaping_sword");
    }
    ...
    public void onMove(SkyblockPlayerMoveEvent e) {
        ...
        if (skyblockPlayer.hasExtraData("leaping_sword")) {
            if (e.getTo().clone().add(0, -1, 0)
                ...
                for (Entity entity : e.getTo().getWorld().getNearbyEntities(e.getTo(), 2, 2, 2)) {
                    ...
                    sentity.damage(Util.calculateAbilityDamage(350, skyblockPlayer.getStat(SkyblockStat.MAX_MANA), 1, skyblockPlayer.getStat(SkyblockStat.ABILITY_DAMAGE)), skyblockPlayer, false);
                }

                skyblockPlayer.setExtraData("leaping_sword", null);
            }
        }
    }
    ...
    public void onRightClick(PlayerInteractEvent event, HashMap<String, Object> data) {
        ...
        if (skyblockPlayer.checkMana(50)) {
            ...
            skyblockPlayer.setCooldown("leaping_sword", 1);

            Util.sendAbility(skyblockPlayer, "Leap", 50);

            Util.delay(() -> {
                skyblockPlayer.setExtraData("leaping_sword", true);
            }, 10);
        }
    }
}
