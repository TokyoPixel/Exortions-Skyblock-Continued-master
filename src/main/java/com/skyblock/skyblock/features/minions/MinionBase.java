...
public abstract class MinionBase {
    ...
    public int[] minionInventorySlots = {21 ,22 , 23 , 24 , 25 , 30 , 31 , 32 ,33 , 34 , 39 , 40 , 41 , 42 , 43};
    ...
    public void pickup(SkyblockPlayer player, Location location) {
        ...
        for (MinionHandler.MinionSerializable minion : island.getMinions()) {
            if (!minion.getBase().getUuid().equals(getUuid())) minions.add(minion);
        }

        island.setMinions(minions);
    ...
    // todo : implement skill xp on collectAll
    public void collectAll(SkyblockPlayer player){
        if (player.getBukkitPlayer().getInventory().firstEmpty() == -1) {
            player.getBukkitPlayer().sendMessage(ChatColor.RED + "Your inventory does not have enough free space to add all items!");
            return;
        }
        for (ItemStack toCollect : inventory){
            if (!Util.notNull(toCollect)) return;
             ItemStack stack = Util.toSkyblockItem(toCollect);
             stack.setAmount(toCollect.getAmount());
             player.getBukkitPlayer().getInventory().addItem(stack);
        }
        for (int slot : minionInventorySlots){
            gui.clear(slot);
            // todo : add check to make sure it minion inventory
           //getOpenPlayerInventory(player.getBukkitPlayer()).clear(slot);
        }
        inventory.clear();
    ...
    public Inventory getOpenPlayerInventory(Player player) {
    ...
    public void createInventory(SkyblockPlayer player){
