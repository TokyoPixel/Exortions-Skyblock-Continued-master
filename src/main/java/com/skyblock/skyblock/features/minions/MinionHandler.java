...
public class MinionHandler {
    ...
    public void reloadPlayer(SkyblockPlayer player, boolean failed) {
        ...
        SkyblockIsland island = SkyblockIsland.getIsland(player);
        int minionsPlaced = ((List<Object>) island.getValue("island.minions")).size();
        if (minionsPlaced != 0) {
            removeStands(island);
        }
        for (MinionSerializable minion : (List<MinionSerializable>) island.getValue("island.minions")) {
            boolean found = false;
            System.out.println("Found minion ");
            String worldName = SkyblockIsland.ISLAND_PREFIX + player.getBukkitPlayer().getUniqueId().toString();

            //Skyblock.getPlugin().sendMessage("Attempting to load player island &8" + worldName + "&f...");


            if (Bukkit.getWorld(worldName) == null) throw new IllegalArgumentException("Minion World is null (" + worldName + ")");

           removeArmorStand(minion.getLocation());
           Skyblock.getPlugin().sendMessage("Successfully removed old minions stands from " + player.getBukkitPlayer().getName() + " 's island");

           removeArmorStand(minion.getLocation().clone().add(0, 1, 0));
            Skyblock.getPlugin().sendMessage("Successfully removed old minions hologram from " + player.getBukkitPlayer().getName() + " 's island");

            minion.getBase().spawn(player, minion.getLocation(), minion.getLevel());

            long secondsSince = ((System.currentTimeMillis() - (long) island.getValue("island.last_login"))) / 1000;
            ...
            Skyblock.getPlugin().sendMessage("Successfully loaded minion &a✔&f in " + player.getBukkitPlayer().getName() + "'s island");
        }
    }
    public void removeStands(SkyblockIsland island) {

        World world = Bukkit.getWorld(SkyblockIsland.ISLAND_PREFIX + island.getUuid());
        ...
        Skyblock.getPlugin().sendMessage("Successfully removed old minions stands from " + world.getName());
    }
    ...
    public void initializeMinion(SkyblockPlayer player, MinionBase minion, Location location) {
        ...
        SkyblockIsland island = SkyblockIsland.getIsland(player);
        island.getMinions().add(serialize);
        island.saveMinions();
    ...
    public void deleteAll(UUID uuid) {
        for (MinionSerializable minion : this.minions.get(uuid)) {
            ...
            List<MinionSerializable> minions = (List<MinionSerializable>) island.getValue("island.minions");
            ...
            island.setValue("island.minions", minions);
        }
    }
    ...
    public void removeArmorStand(Location location) {
        ...
        if (armorStand != null) {
            armorStand.remove();
            Bukkit.getLogger().info("Armor stand removed at location: " + location.toString());
        }
    }
