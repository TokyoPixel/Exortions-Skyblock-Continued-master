...
import com.skyblock.skyblock.events.*;
...
//        if (!event.getFrom().getName().startsWith(SkyblockIsland.ISLAND_PREFIX)) {
...
//                }
                ...
                Util.delay(() -> {
                    ...
                    player.teleport(new Location(Skyblock.getSkyblockWorld(), -2, 70, -84, -180, 0));
                }, 1);
        ...
        new BukkitRunnable(){
            ...
            public void run() {
                if (player.isOnline()){
                    SkyblockPlayer.getPlayer(player).statsTick();
                }else{
                    cancel();
                }
            }
    ...
    @EventHandler
    public void onMove(SkyblockPlayerMoveEvent event) {
        ...
        Location to = event.getTo();
            ...
            if (location != null && skyblockPlayer.getExtraData("last_location") != location.getName() && !player.getWorld().getName().startsWith(SkyblockIsland.ISLAND_PREFIX)) {
    ...
    public void onQuit(PlayerQuitEvent event) {
        SkyblockPlayer player = SkyblockPlayer.getPlayer(event.getPlayer());
        for (ItemStack item : player.getBukkitPlayer().getInventory().getArmorContents()) {
                    player.updateStats(item, null); // remove armor and hand stats on quit! so it wont get doubled on join!
        }
        if (player.getBukkitPlayer().getItemInHand() != null) {
            player.updateStats(player.getBukkitPlayer().getItemInHand() , null);
        }
...
//    }
    ...
    public void onMove(PlayerMoveEvent e){
        if(e.getFrom().getBlockX() != e.getTo().getBlockX() || e.getFrom().getBlockY() != e.getTo().getBlockY() || e.getFrom().getBlockZ() != e.getTo().getBlockZ()) {
            SkyblockPlayerMoveEvent moveEvent = new SkyblockPlayerMoveEvent(e.getPlayer() , e.getFrom() , e.getTo());
