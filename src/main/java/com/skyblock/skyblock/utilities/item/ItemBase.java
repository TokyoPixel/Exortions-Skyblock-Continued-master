...
public class ItemBase {
    ...
    public ItemStack createStack() {
        ...
        if (strength != 0 || rStrength > 0) {
            String strengthString = ChatColor.GRAY + "Strength: " + ChatColor.RED + "+" + (strength + rStrength);
            if (reforge != Reforge.NONE && rStrength > 0 || isThick) {
                strengthString += " " + ChatColor.BLUE + "(+" + rStrength + (isThick ? 100 : 0) + ")";
            }
            lore.add(strengthString);
        }
        if (critChance != 0 || rCritChance > 0) {
            String critChanceString = ChatColor.GRAY + "Crit Chance: " + ChatColor.RED + "+" + (critChance + rCritChance) + "%";
            if (reforge != Reforge.NONE && rCritChance > 0) {
                critChanceString += " " + ChatColor.BLUE + "(+" + rCritChance + "%)";
            }
            lore.add(critChanceString);
        }
        if (critDamage != 0 || rCritDamage > 0) {
            String critDamageString = ChatColor.GRAY + "Crit Damage: " + ChatColor.RED + "+" + (critDamage + rCritDamage) + "%";
            if (rCritDamage > 0) {
                critDamageString += " " + ChatColor.BLUE + "(+" + rCritDamage + "%)";
            }
            lore.add(critDamageString);
        }
        if (attackSpeed != 0 || rAttackSpeed > 0) {
            String attackSpeedString = ChatColor.GRAY + "Attack Speed: " + ChatColor.RED + "+" + (attackSpeed + rAttackSpeed) + "%";
            if (reforge != Reforge.NONE && rAttackSpeed > 0) {
                attackSpeedString += " " + ChatColor.BLUE + "(+" + rAttackSpeed + "%)";
            }
            lore.add(attackSpeedString);
        }

        if ((speed != 0 || rSpeed != 0) || (intelligence != 0 || rMana != 0) || (defense != 0 || rDefense != 0) || (health != 0 || rHealth > 0)) {
            lore.add("");

            if (health != 0 || rHealth > 0) {
                String healthString = ChatColor.GRAY + "Health: " + ChatColor.GREEN + "+" + (health + rHealth);
                if (reforge != Reforge.NONE && rHealth > 0) {
                    healthString += " HP " + ChatColor.BLUE + "(+" + rHealth + ")";
                }
                lore.add(healthString);
            }

            if (defense != 0 || rDefense != 0) {
                String defenseString = ChatColor.GRAY + "Defense: " + ChatColor.GREEN + "+" + (defense + rDefense);
                if (reforge != Reforge.NONE && rDefense > 0) {
                    defenseString += " " + ChatColor.BLUE + "(+" + rDefense + ")";
                }
                lore.add(defenseString);
            }

            if (speed != 0 || rSpeed != 0) {
                String speedString = ChatColor.GRAY + "Speed: " + ChatColor.GREEN + "+" + (speed + rSpeed);
                if (reforge != Reforge.NONE && rSpeed > 0) {
                    speedString += " " + ChatColor.BLUE + "(+" + rSpeed + ")";
                }
                lore.add(speedString);
            }

            if (intelligence != 0 || rMana != 0) {
                String intelligenceString = ChatColor.GRAY + "Intelligence: " + ChatColor.GREEN + "+" + (intelligence + rMana);
                if (reforge != Reforge.NONE && rMana > 0) {
                    intelligenceString += " " + ChatColor.BLUE + "(+" + rMana + ")";
                }
                lore.add(intelligenceString);
            }
        }
